...
void iter_nFindSol(node *head){
  ...
  clos_path *h2=NULL,*h3=NULL,*temp2;
  node *k=head,*l=head;
...
int findNum(int a[M+N],int i,int j){
  /*
     This function either returns which of the i, j is present in the array a[M+N]
                   or the position of the array at which i is present as per the calling situation
  */
  int k=0;
  if(i<0 && j<0){
    while(k<M+N)
